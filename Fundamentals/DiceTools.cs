...
{
    // The shared source is thread-safe; supplied sources retain their own concurrency contract.
    private static readonly IRandomSource randomSource = new SystemRandomSource();

    // Validation messages are shared by single-group and multi-group roll overloads.
    ...
    // Callers validate that sides + 1 fits the random source's exclusive upper bound.
    private static int RollOneDieTrustedArgs(int sides, IRandomSource source)
    {
        return source.Next(1, sides + 1);
    }

    // Callers validate the group once; accumulation still detects integer overflow.
