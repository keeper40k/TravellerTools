...
{
    // Display rows retain literal line feeds for existing consumers.
    ...
    public TravellerService()
    {
        Name = string.Empty;
        Enlistment = new(0);
        EnlistmentPlusOne = new(string.Empty, 0);
        EnlistmentPlusTwo = new(string.Empty, 0);
        Survival = new(0);
        SurvivalPlusTwo = new(string.Empty, 0);
        Commission = new(0);
        CommissionPlusOne = new(string.Empty, 0);
        Promotion = new(0);
        PromotionPlusOne = new(string.Empty, 0);
        Reenlist = new(0);
        DraftNumber = 0;
        Ranks = new();
        AutomaticSkills = new();
        ...
        PersonalDevelopmentTable = new();
        ServiceSkillsTable = new();
        AdvancedEducationTable = new();
        AdvancedEducationTable2 = new();
        CashTable = new();
        BenefitsTable = new();
    }
    ...
    /// <remarks>Null adjustments are skipped. Characteristic adjustments include a sign; negative signs and numbers use the current culture.</remarks>
    protected string TableText(List<KeyValuePair<int, TravellerSkillModifier>> table)
    {
        ...
        {
            TravellerSkillModifier? adjustment = row.Value;
            if (adjustment != null)
            {
                if (adjustment.IsSkill)
                {
                    result += string.Format(SkillRowFormat, row.Key, adjustment.Name);
                }
                else if (adjustment.IsAttribute)
                {
                    // Numeric formatting supplies the negative sign, including for Int32.MinValue.
                    string modifier = adjustment.Level < 0 ? string.Empty : "+";
                    result += string.Format(AttributeRowFormat, row.Key, modifier, adjustment.Level, adjustment.Name);
                }
            }
        }
        // Remove the last \n character
    ...
    public string RankName(int rankIndex)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(rankIndex), "Parameter rank must be within the range of available ranks.");
        }
    ...
    {
        List<TravellerSkillModifier> results = new();
